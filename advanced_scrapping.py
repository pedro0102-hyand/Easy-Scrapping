...
def extrair_citacoes(html):
    ...
    for quote in soup.find_all('div', class_='quote'):
        ...
        link_autor = quote.find('a')['href']

        citacoes.append({'texto': texto, 'autor': autor, 'tags': tags, 'link_autor': urljoin(URL, link_autor)})
...
def main():
    ...
    for c in citacoes:
        print(f"{c['texto']} - {c['autor']} - Tags: {', '.join(c['tags'])} - Link do autor: {c['link_autor']}")
